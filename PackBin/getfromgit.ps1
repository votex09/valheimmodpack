...
if ($args[0] -eq "-update") 
{
    ...
    { 
        ...
        if (Test-Path -Path "$PSScriptRoot\PackBin\git\valheimdirtbagmodpack\PackBin\pack.ini") 
        {
            ...
            {
                #Write-Host "$($mod.name) - Remote: v$($mod.value.version) Local: v$($localModList.BepInEx)" -ForegroundColor Yellow
                if (!($localModList."$($mod.name)") -or ($localModList.$($mod.name) -ne $mod.value.version))
                {
                ...
                {
                    #normal reinstall previous package
                    Get-DLMethod $mod.name $mod.value.method "reinstall" $mod.value.version $false $mod.value.urlsuffix
                }
            }
